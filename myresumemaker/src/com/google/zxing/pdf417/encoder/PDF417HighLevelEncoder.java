...
final class PDF417HighLevelEncoder
{
    ...
    throws WriterException
  {
    ...
    while (true)
    {
      ...
      {
        Compaction localCompaction = Compaction.NUMERIC;
        arrayOfByte1 = null;
        j = 0;
        k = 0;
        m = 0;
        if (paramCompaction != localCompaction)
          break;
    ...
    while (k < i)
    {
      int n = determineConsecutiveDigitCount(paramString, k);
      if (n >= 13)
      {
        localStringBuilder.append('Ά');
        j = 2;
        encodeNumeric(paramString, k, n, localStringBuilder);
        k += n;
        m = 0;
      }
      else
      {
        int i1 = determineConsecutiveTextCount(paramString, k);
        if ((i1 >= 5) || (n == i))
        {
          if (j != 0)
          {
          ...
          m = encodeText(paramString, k, i1, localStringBuilder, m);
          k += i1;
        }
        else
        {
          ...
          i2 = determineConsecutiveBinaryCount(paramString, arrayOfByte1, k);
          if (i2 == 0)
            i2 = 1;
          if ((i2 != 1) || (j != 0))
            break label282;
          encodeBinary(arrayOfByte1, k, 1, 0, localStringBuilder);
        }
      }
    }
    while (true)
    {
      ...
      break label106;
      break;
      label282: encodeBinary(arrayOfByte1, k, i2, j, localStringBuilder);
      j = 1;
      m = 0;
    }
  }
